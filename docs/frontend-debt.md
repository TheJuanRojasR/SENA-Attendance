...
| 4 | El backend ya expone `mustChangePassword` en el `AdminUserDTO` (`GET /api/account` y respuestas admin); las cuentas creadas por un Administrador nacen en `true`. Falta que `POST /api/authenticate` devuelva el indicador en el login y queda pendiente el flujo de cambio obligatorio de contraseña. | `Pendiente` |
