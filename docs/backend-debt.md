...
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ----------- |
| UC003 — Modificar datos              | Teléfono sin validación de 10 dígitos y correo no obligatorio; no hay rechazo explícito al intentar modificar el documento (E3). El cambio de contraseña (E4–E6) ya quedó cubierto por las unidades de UC002.                                  | `web/rest/vm/AccountUpdateVM.java:23-24` (solo `@Size`, sin `@Pattern`); `service/UserService.java:622-693`               | `Pendiente` |
...
| UC005 — Recuperar contraseña         | Clave de reset inválida responde `500` (debería `400`); la clave expira a las 24 h (el UC pide 30 min); `reset-finish` solo valida longitud, no la política completa. El correo se envía.                                                      | `web/rest/AccountResource.java:148-152`, `:155-161`; `service/UserService.java:97-109`                                    | `Pendiente` |
...
| UC019 — Configuración global         | El modelo solo expone dos de los cuatro parámetros: faltan `consecutiveAbsenceAlertThreshold` y `accumulatedAbsenceAlertThreshold` que exige UC013.                                                                                            | `domain/GlobalConfiguration.java:23-33`; `service/dto/GlobalConfigurationDTO.java:15-19`                                 | `Pendiente` |
| UC020 — Gestionar jornadas           | Sin nombre único (E1), sin validar horas distintas (E2), sin bloquear la eliminación en uso (E3) ni el rango con cruce de medianoche.                                                                                                          | `docs/api-contracts.md` — UC020; `web/rest/TimeSlotResource.java:55-169`                                                 | `Pendiente` |
| UC021 — Gestionar modalidades        | Sin unicidad de nombre (E1) ni bloqueo de eliminación en uso (E2). El estado se maneja con `isActive`, no con acciones Desactivar/Reactivar.                                                                                                  | `docs/api-contracts.md` — UC021; `web/rest/ModalityResource.java:55-169`                                                 | `Pendiente` |
| UC022 — Tipos de documento           | Sin validar nombre ni iniciales duplicados (E1/E2), sin bloquear el cambio de iniciales de un tipo en uso (E3) ni la eliminación por uso (E4). El estado `isActive` ya existe.                                                                 | `service/dto/DocumentTypeDTO.java:23`; `web/rest/DocumentTypeResource.java:54-177`; `docs/api-contracts.md` — UC022       | `Pendiente` |
...
| UC013 — Alertas de inasistencia      | No implementada: falta la entidad `Alerta`, los umbrales en `GlobalConfiguration`, la evaluación al guardar asistencia o aprobar justificaciones y las notificaciones asociadas.                                                                | `docs/api-contracts.md` — UC013; `domain/enumeration/NotificacionTipo.java:7`                                             | `Pendiente` |
...
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ----------- |
| `@PreAuthorize` con `ROLE_COORDINATOR`                           | Varios resources siguen aceptando un rol que los casos de uso ya no contemplan; debe quedar solo `ROLE_ADMIN` (u operativo donde aplique).            | `web/rest/DocumentTypeResource.java:55`; `TimeSlotResource.java:55`; `ModalityResource.java:55`; `ProgramResource.java:63`; `TrimesterResource.java:61` | `Pendiente` |
...
| `GlobalConfiguration` con 2 de 4 parámetros                      | Faltan los dos umbrales de alerta de UC013.                                                                                                          | `domain/GlobalConfiguration.java:23-33`                                                                            | `Pendiente` |
