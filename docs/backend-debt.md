...
| UC014 — Trimestres académicos     | Resuelto: `PUT` y `PATCH` aplican las mismas reglas de estado, orden, solape y asistencias; `DELETE` bloquea si hay horarios o asistencias (E6, `error.trimesterInUse`); escrituras solo `ROLE_ADMIN`; mensajes E1/E3/E4 alineados. **Pendiente**: al crear no se exige fecha inicio ≥ mañana ni fecha fin ≥ hoy (E2), y `status` es booleano, así que no se puede filtrar por Futuro/Activo/Cerrado ni hay `/active`. | `docs/api-contracts.md` — UC014; `service/impl/TrimesterServiceImpl.java`                                             | `Pendiente` |
...
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `@PreAuthorize` con `ROLE_COORDINATOR`              | Varios resources siguen aceptando un rol que los casos de uso ya no contemplan; debe quedar solo `ROLE_ADMIN` (u operativo donde aplique). Ya restringidos a `ROLE_ADMIN`: DocumentType, TimeSlot, Modality, JustificationType y Program. | `web/rest/TrimesterResource.java:61`                                                                                         | `Pendiente` |
