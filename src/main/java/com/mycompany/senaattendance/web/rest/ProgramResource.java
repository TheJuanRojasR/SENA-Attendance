...
/**
 * REST controller for managing {@link com.mycompany.senaattendance.domain.Program}.
 */
@RestController
...
public class ProgramResource {
    ...
    public ResponseEntity<ProgramDTO> createProgram(@Valid @RequestBody ProgramDTO programDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/programs/" + programDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, programDTO.getId()))
            .body(programDTO);
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<ProgramDTO> getProgram(@PathVariable("id") String id) {
